...
static THD_FUNCTION(ProximityDetection, arg){
	...
	while(1){
		...
		if(get_selector() == SELECT_START){
			...
			if(get_calibrated_prox(1) < PROX_FALL_VALUE && get_function_mode() == NORMAL_MODE){
				set_function_mode(FALL_MODE);
			}
			...
			if((get_calibrated_prox(0) > PROX_FLOOR_DETECT || get_calibrated_prox(1) > PROX_FLOOR_DETECT)
				&& (get_function_mode() == FALL_MODE || get_function_mode() == PARABOLA_MODE)){
				set_function_mode(LANDING_MODE);
			}


			// switch from normal mode to end mode if end detected with IR6
			if(get_calibrated_prox(5) > PROX_END_DETECT && get_function_mode() == NORMAL_MODE){
				set_function_mode(END_MODE);
			}
		}
